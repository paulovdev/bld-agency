...
const CollabBlock = ({
  ...
  return (
    <motion.div
      className={`relative h-screen flex ${reversed ? "flex-row-reverse" : ""}`}
      id={title.toLowerCase().replace(/\s+/g, "-")}
    >
      <div className="relative flex flex-[calc(25%_+_20px)] max-lg:flex-[calc(25%_+_5px)]">
        <Image
          src={src}
          width={1000}
          height={1000}
          alt={`${title} Collaboration`}
          className="w-full h-screen object-cover"
        />
      </div>

      <div className="pl-10 h-full flex-[2] flex flex-col justify-center max-lg:pl-3">
        <div className="mb-8">{logo}</div>

        <h2 className="text-[6vw] text-p font-bigger leading-[1] tracking-wide uppercase">
          {title}
        </h2>

        <h3 className="mt-2 text-[1rem] text-p uppercase tracking-[1px] font-semibold max-md:text-[.8rem]">
          {subTitle}
        </h3>

        <p className="my-8 text-[.85rem] text-p/75 max-w-[500px] leading-tight">
          {description}
        </p>

        <button
          className="w-fit px-8 h-[42px] rounded-full bg-bg-2 flex items-center justify-center border border-brd hover:bg-brd transition-colors 
        max-lg:h-[32px]"
        >
          <span className="text-s text-[.8rem] tracking-wide max-lg:text-[.75rem]">
            {cta}
          </span>
        </button>
      </div>
    ...
    <>
      <div id="truein ">
        <CollabBlock
          ...
          title="Building brands with TrueIn"
          subTitle="Authenticity in action"
          description="TrueIn™ is the engine behind the collaboration — where instinct meets clarity. We peeled back layers to surface what matters, crafting identity systems that feel real, resonate loud, and stand the test of noise."
          cta="SEE TRUEIN™"
          src="/photos/home/img-12.jpg"
        />
      </div>
      <div id="nova">
        <CollabBlock
          ...
          title="Shaping tomorrow’s NOVA COLLECTIVE"
          subTitle="Co-created futurewear"
          description="With Nova Collective, we fused visionary design with responsible execution. This collab reimagined supply chains as storytelling platforms, turning sustainable intent into bold visual language that moves culture forward."
          cta="SEE NOVA COLLECTIVE"
          src="/photos/home/img-13.jpg"
          reversed
        />
      </div>
      <div id="echo">
        <CollabBlock
          ...
          title="Amplifying brand resonance"
          subTitle="Data-driven disruption"
          description="Echo Labs challenged us to make insight sound like signal. Together we engineered campaigns that don’t whisper — they echo. Strategy informed every frame, making sure the message reverberates long after the first impression."
          cta="EXPLORE ECHO"
          src="/photos/home/img-11.jpg"
        />
      </div>
    </>
  );
